...
function install() {
  ...
  // Normalize both URLs before comparing: strip trailing .git and trailing slash
  function normalizeUrl(u) {
    return u.replace(/\.git$/, '').replace(/\/$/, '');
  }
  if (normalizeUrl(originUrl) !== normalizeUrl(REPO_URL)) {
  ...
  const isSigned = verifyOutput.includes('gpg: Signature') ||
                   verifyOutput.includes('Good "git" signature');
  const isBad = verifyOutput.includes('gpg: BAD signature') ||
                verifyOutput.includes('BAD signature');
  if (!isSigned || isBad) {
  ...
  // Try GPG format first: "gpg: Signature made ... using RSA key XXXXXXXX"
  // Then SSH format: "Good "git" signature for ... with ED25519 key SHA256:XXXX"
  const gpgSignatureLine = verifyOutput.split('\n').find(l => l.startsWith('gpg: '));
  const sshSignatureLine = verifyOutput.split('\n').find(l => l.includes('Good "git" signature'));
  let commitHash = 'unknown';
  let sigType = 'unknown';
  if (gpgSignatureLine) {
    commitHash = (verifyOutput.match(/^([a-f0-9]{40})\s/m) || [])[1] || 'unknown';
    sigType = 'GPG-signed';
  } else if (sshSignatureLine) {
    commitHash = (verifyOutput.match(/^([a-f0-9]{40})\s/m) || [])[1] || 'unknown';
    sigType = 'SSH-signed';
  }
  console.log(`   Verified commit: ${commitHash} (${sigType})`);
